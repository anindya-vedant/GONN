...
def test(stou):
    ...
    for i in range(len(dataTest)):
        label_test = dataTest.values[i,-1]
        stin = stou.copy()
        for k in range(len(stin)):
            if stin[k] not in ['+', '-', '*', '/'] and type(stin[k]) == int:
        ...
        # if val >= 0.5:
        #     print(type(dataTest), "Sahibnoor")
        #     if dataTest.values[i, -1] == 0:
        #         tp = tp + 1
        #         c1 = c1 + (sig(val) ** 2)
        #     else:
        #         fp = fp + 1
        # else:
        #     if dataTest.values[i, -1] == 1:
        #         fn = fn + 1
        #     else:
        #         tn = tn + 1
        #         c2 = c2 + (sig(val) ** 2)
...
test_accuracy_list.sort(reverse=True)
print((accuracy_list[0])/len(dataTrain)*100)
print((test_accuracy_list[0]/len(dataTest))*100)
# Confusion Matrix
...
filename = 'finalized_model_long.pkl'
filename_hickle = 'finalized_model_long.hkl'
pickle.dump(preorder_trees, open(filename, 'wb'))
print(accuracy_list)
print(test_accuracy_list)
# hickle.dump(preorder_trees, 'test.hkl', mode='w')
